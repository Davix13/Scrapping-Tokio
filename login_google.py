...
while numero <= len(url):
    ...
    streams = v.streams  # List of available streams of different quality
    # >> [Stream(240p), Stream(360p), Stream(540p), Stream(720p), Stream(1080p)]

    # Download best stream
    ruta = str(video[numero][31:])
    try:
        # Descargar el mejor stream
        best_stream = streams[-1]
        best_stream.download(download_directory='video', filename=ruta)

    except Exception as e:
        # Manejar la excepción de manera más específica
        print(f"Error al descargar el video {ruta}: {e}")
    numero += 1
